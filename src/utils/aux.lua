...
function aux.show_congratulation()
  ...
  local final_messages = {
    ...
    "⚡ Afio Arch instalado e configurado!",
  ...
  if envs.DYNAMIC.CPU_VENDOR then
    ...
    print(envs.COLORS.DIM .. "    Drivers: " .. (envs.DYNAMIC.CPU_VENDOR == "AMD" and "amd-ucode, vulkan-swrast" or "intel-ucode") .. envs.COLORS.RESET)
  else
  ...
  if envs.DYNAMIC.GPU_VENDORS and #envs.DYNAMIC.GPU_VENDORS > 0 then
    print(envs.COLORS.GREEN .. "  ✓ Placas Gráficas (GPU): " .. table.concat(envs.DYNAMIC.GPU_VENDORS, ", ") .. envs.COLORS.RESET)
  ...
  print(envs.COLORS.DIM .. "    └─ Hook automático criado (próximas atualizações serão automáticas)" .. envs.COLORS.RESET)
  print()
  ...
  print(envs.COLORS.BLUE .. string.rep("═", 70) .. envs.COLORS.RESET)
  print()
  print(envs.COLORS.BOLD .. envs.COLORS.YELLOW .. "  Obrigado por usar Afio Arch!" .. envs.COLORS.RESET)
...
function aux.save_config_backup()
  ...
  json_string = json_string .. '    "display_server": "' .. (backup_data.system_config.display_server or "none") .. '",\n'
  json_string = json_string .. '    "cpu_vendor": "' .. (backup_data.system_config.cpu_vendor or "none") .. '",\n'
  json_string = json_string .. '    "gpu_vendors": ["' .. (backup_data.system_config.gpu_vendors and table.concat(backup_data.system_config.gpu_vendors, '", "') or "") .. '"],\n'
  json_string = json_string .. '    "desktop_environment": "' .. (backup_data.system_config.desktop_environment or "none") .. '",\n'
  json_string = json_string .. '  },\n'
...
aux.groups_categories = {
  ...
  ["Desenvolvimento"]        = { "DEVELOPMENT", "PYTHON", "NODE", "RUST", "ENGINEERING" },
...
function aux.confirm(prompt)
  ...
  local cmd = string.format(
    'gum confirm "%s" --affirmative="Confirmar" --negative="Cancelar"',
    prompt
  )
  local result = os.execute(cmd)
  return result == true or result == 0
...
function aux.calculate_sizing_total()
  ...
  if envs.DYNAMIC.PACKAGES_INSTALLED then
    for _, cat in pairs(envs.DYNAMIC.PACKAGES_INSTALLED) do
      for _, pkg in ipairs(cat) do table.insert(all_pkgs, pkg) end
    end
  end

  if #all_pkgs == 0 then return "0 MB" end

  local awk_script = "'/^Installed Size/ { if ($5 == \"MiB\") sum += $4; else if ($5 == \"KiB\") sum += $4/1024; else if ($5 == \"GiB\") sum += $4*1024 } END { printf \"%.2f MB\\n\", sum }'"
...
function aux.choose_per_number(prompt, options)
  ...
  local cmd = string.format(
    'gum choose --height=%d --header="%s" %s',
    list_height,
    prompt,
...
function aux.multiselect(prompt, options)
  ...
  local cmd = string.format(
    'gum choose --no-limit --height=%d --header="%s" %s',
    list_height,
    prompt,
    opts_str
  )
