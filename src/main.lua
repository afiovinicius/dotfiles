...
local desktopConfig = require("src.services.desktop_config")
...
local function main()
	...
	if aux.confirm("Deseja iniciar a instalação do Orzhov Arch?") then
		...
		print(
			envs.COLORS.BOLD .. envs.COLORS.GREEN .. "✅ BASE DO SISTEMA INSTALADA COM SUCESSO!" .. envs.COLORS.RESET
		)
		print()
		...
		print(
			envs.COLORS.CYAN
				.. "  • Display Server: "
				.. (envs.DYNAMIC.DISPLAY_SERVER or "não selecionado")
				.. envs.COLORS.RESET
		)
		print()
	...
	if aux.confirm("Deseja iniciar as configurações do desktop?") then
		if not desktopConfig.config_desktop() then
			aux.pf("Erro ao tentar configurar o dektop", "error")
		end
	else
		print()
		aux.pf("Pulando configurações do desktop...", "warn")
		return
	end
