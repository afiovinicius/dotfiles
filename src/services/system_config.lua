--- ============================================================================
--- AFIO ARCH - CONFIGURAÇÕES PADRÕES PARA O SISTEMA
--- ============================================================================
...
function systemConfig.config_memory()
  ...
  if not success_swapiness then
    aux.pf("[SWAPINESS & ZRAM] Erro ao criar configuração do Sysctl: " .. tostring(create_error_swapiness), "error")
    return false
  end
  ...
  aux.run_cmd_valid("sudo systemctl restart systemd-zram-setup@zram0.service", "[SWAPINESS & ZRAM] Reiniciando serviço ZRAM")
